...
class RecTrainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, rgb, target) in enumerate(tqdm(self.data_loader)):
            ...
            get output and loss
            """
            if self.config["arch"]["args"]["in_channels"] == 4:
                data = torch.cat((data, rgb),1)
            
            output = self.model(data, torch.ones_like(data))
            output_2 = self.model(data, output)
            
            loss = self.criterion(output_2, target)
            loss.backward()
            self.optimizer.step()
